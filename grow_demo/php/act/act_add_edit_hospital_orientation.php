...
/* Action file for editing staff details */
	...
	if($GroupID != '')
	{
		$Group = Business\HospitalOrientation::Load($GroupID);
		...
		if($intSubmitted == 1)
		{
			...
			if($blnIsGood)
			{
				...
				if($GroupID != '')
				{
					...
					$Group = Business\HospitalOrientation::Create($TeamName,$SafeSD,$SafeED);
					
					Auditing\ChangeLog::CreateChangeLog($_SESSION['User']->GetUserID(),$page_id,$Group->GetGroupID(),CHANGE_INSERT,'tbl_groups');
				}
				
				$go_to_url = $lnk_view_hospital_orientations_secure.'#group_'.$GroupID;
