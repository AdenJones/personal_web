							...
							if($this_staff == false)
								{
									...
									if($secure)
									{
										$this_url = '<a href="'.$lnk_add_edit_staff.'&id_user='.$this_staff->GetUserID().'&return_to=view_staff_new'.'">';
										$this_url_end = '</a>';
										
										$str_class = ($this_staff->HasLogin()) ? 'edit' : 'add';
										$this_add_edit = ($this_staff->HasLogin()) ? 'Edit' : 'Add';
										
										$this_edit_login =  '<a class="'.$str_class.'" href="'.$lnk_add_edit_staff_login.'&id_user='.$this_staff->GetUserID().'&return_to=view_staff_new'.'">'.$this_add_edit.' Login</a>';
										
										$this_view_dates = '<div class="form_item spaced"><a href="'.$lnk_view_staff_volunteer_dates_secure.'&id_user='.$this_staff->GetUserID().'&return_to=view_staff_new'.'">View All Activity Dates</a></div>';
										...
										if( $this_staff->HasLogin() and $this_staff->GetUserTypeName() == 'Field Worker' )
										{
											...
											$this_url_regions = $lnk_view_staff_regions_secure.'&id_user='.$this_staff->GetUserID().'&return_to=view_staff_new';
										...
										$this_reminder_dates = '<div class="form_item spaced"><span class="label">Reminder Dates: </span><a href="'.$lnk_view_staff_reminder_dates.'&id_user='.$this_staff->GetUserID().'&return_to=view_staff_new'.'">View Reminder Dates</a></div>';
										if( $this_staff->GetUserTypeName() == $StateUser )
										{
											$this_state_activity_dates = '<div class="form_item spaced"><span class="label">State Activity Dates: </span><a href="'.$lnk_view_state_user_states_secure.'&id_user='.$this_staff->GetUserID().'&return_to=view_staff_new'.'">View State Activity Dates</a></div>';
										}
										else 
										{
