	...
	if($intSubmitted == 1)
	{
		...
		if( $blnIsGood )
		{
			...
			if($TypeSelected == GROUP and $HasGroup)
			{
				...
				$Report = $_SESSION['User']->AddReport(ATTENDEES_BY_GROUP,$thisGroup->GetGroupName(),funAusDateFormat($str_safe_s_date).' - '.funAusDateFormat($str_safe_e_date),$_SESSION['User']->GetUserID());
				
				$ReportID = $Report->GetIDUserReport();
				
				//include '../php/async/async_attendees_by_group.php';
				
				$cmd = "php ".$BaseIncludeURL."/async/async_attendees_by_group.php ".$GroupID." ".$StartDate." ".$EndDate." ".$ReportID." ".$_SESSION['User']->GetUserID();
			...
			elseif($TypeSelected == REGION and $HasRegion)
			{
				...
				$Report = $_SESSION['User']->AddReport(ATTENDEES_BY_REGION,$ReportName,funAusDateFormat($str_safe_s_date).' - '.funAusDateFormat($str_safe_e_date),$_SESSION['User']->GetUserID());
			
				$ReportID = $Report->GetIDUserReport();
				
				//include '../php/async/async_attendees_by_region.php';
				$cmd = "php ".$BaseIncludeURL."/async/async_attendees_by_region.php ".$RegionID." ".$StartDate." ".$EndDate." ".$ReportID." ".$_SESSION['User']->GetUserID();
			...
			elseif($TypeSelected == BRANCH and $HasBranch)
			{
				...
				$Report = $_SESSION['User']->AddReport(ATTENDEES_BY_BRANCH,$ReportName,funAusDateFormat($str_safe_s_date).' - '.funAusDateFormat($str_safe_e_date),$_SESSION['User']->GetUserID());
				
				$ReportID = $Report->GetIDUserReport();
				
				//include '../php/async/async_attendees_by_branch.php';
				$cmd = "php ".$BaseIncludeURL."/async/async_attendees_by_branch.php ".$Branch." ".$StartDate." ".$EndDate." ".$ReportID." ".$_SESSION['User']->GetUserID();
