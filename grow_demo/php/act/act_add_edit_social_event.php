...
/* Action file for editing staff details */
	...
	if($GroupID != '')
	{
		$Group = Business\SocialEvent::Load($GroupID);
		...
		if($intSubmitted == 1)
		{
			...
			if($blnIsGood)
			{
				...
				if($GroupID != '')
				{
					...
					$NewGroup = Business\SocialEvent::Create($TeamName,$SafeSD,$SafeED);
					
					$GroupID = $NewGroup->GetGroupID();
					
					Auditing\ChangeLog::CreateChangeLog($_SESSION['User']->GetUserID(),$page_id,$NewGroup->GetGroupID(),CHANGE_INSERT,'tbl_groups');
				}
				
				$go_to_url = $lnk_view_social_events_secure.'#group_'.$GroupID;
