			<div class="generic_form">
            	<div class="form_major_heading"><h1><?php echo $page_name; ?> : <?php echo $member->GetFirstName().' '.$member->GetLastname(); ?></h1></div>
           		<form action="<?php echo "$full_uri/index.php"; ?>" method="post">
                	...
                	<input type="hidden" name="page_id" value="add_edit_member_committed_dates" />
                    ...
                    <?php if($CommittedID != ''){ echo '<input type="hidden" name="id_committed" value="'.$CommittedID.'" />'; } ?>
                    <?php if($ReturnImmediate != ''){ echo '<input type="hidden" name="return_immediate" value="'.$ReturnImmediate.'" />'; } ?>
                    
							<div class="form_row">
								<div class="form_heading"><h2>Member Committed Dates</h2></div>
