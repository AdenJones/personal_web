	...
	if($thisUser == NULL)
	{
		...
		if($intSubmitted == 1)
		{
			...
			if( $blnIsGood )
			{
					...
					$Total = 0;
					//Attendees Details
					$AttMembers = count($GroupObject->LoadMemberAttendeesBetweenDates($str_safe_s_date,$str_safe_e_date));
					$Total += $AttMembers;
					Business\UserReportField::Create($ReportID,'Total Members:',$AttMembers);
					$AttOrganisers = count($GroupObject->LoadVolunteerAttendeesBetweenDates(ORGANISER,$str_safe_s_date,$str_safe_e_date));
					$Total += $AttOrganisers;
					Business\UserReportField::Create($ReportID,'Total Organisers:',$AttOrganisers);
					$AttRecorders = count($GroupObject->LoadVolunteerAttendeesBetweenDates(RECORDER,$str_safe_s_date,$str_safe_e_date));
					$Total += $AttRecorders;
					Business\UserReportField::Create($ReportID,'Total Records:',$AttRecorders);
					$AttSponsors = count($GroupObject->LoadVolunteerAttendeesBetweenDates(SPONSOR,$str_safe_s_date,$str_safe_e_date));
					$Total += $AttSponsors;
					Business\UserReportField::Create($ReportID,'Total Sponsors:',$AttSponsors);
					...
					Business\UserReportField::Create($ReportID,'Total Community Observers:',$AttCommunityObservers);
					Business\UserReportField::Create($ReportID,'Total Attendees:',$Total);
