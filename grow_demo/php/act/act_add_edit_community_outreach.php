...
/* Action file for editing staff details */
	...
	if($GroupID != '')
	{
		$Group = Business\CommunityOutreach::Load($GroupID);
		...
		if($intSubmitted == 1)
		{
			...
			if($blnIsGood)
			{
				...
				if($GroupID != '')
				{
					...
					$NewGroup = Business\CommunityOutreach::Create($TeamName,$SafeSD,$SafeED);
				...
				$go_to_url = $lnk_view_community_outreach_secure.'#group_'.$GroupID;
